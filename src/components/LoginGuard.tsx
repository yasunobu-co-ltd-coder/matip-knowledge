...
export default function LoginGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const pinOk = !VALID_PIN || sessionStorage.getItem("matip_knowledge_pin_verified") === "true";
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (pin === VALID_PIN) {
      sessionStorage.setItem("matip_knowledge_pin_verified", "true");
      setError("");

      const stored = localStorage.getItem("matip-knowledge-user");
      if (stored) {
        try {
