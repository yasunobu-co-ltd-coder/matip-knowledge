...
- 「カレンダーに追加して」「○月○日に予定を入れて」→ add_calendar_event を使う
...
const TOOLS: OpenAI.ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      ...
      description: "カレンダーに予定を追加する。日付と内容を指定してメモとして登録し、カレンダーに表示されるようにする。",
      parameters: {
        type: "object",
        properties: {
          date: { type: "string", description: "予定の日付（YYYY-MM-DD形式）" },
          content: { type: "string", description: "予定の内容" },
          importance: { type: "string", enum: ["高", "中", "低"], description: "重要度（デフォルト: 中）" },
        },
